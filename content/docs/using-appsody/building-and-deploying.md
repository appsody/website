...
If --knative is used along with --generate-only, the value of `createKnativeService` will be set to `true` in the generated app-deploy.yaml file.  HOWEVER, if during a subsequent appsody deploy call the --knative flag is not present, the value of `createKnativeService` will be set to `false` in app-deploy.yaml.
...
#### Coexistence of an appsody operator created by appsody operator install or appsody deploy with appsody operators created by other means

- Only one appsody operator per namespace is allowed when created by `appsody operator install` or `appsody deploy`.  The code for `appsody operator install` and `appsody deploy` ensures this is the case.  If an appsody operator is created by another means, it is up to the user to make sure that only one appsody operator exists in the given namespace.
