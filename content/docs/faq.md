...
The [Appsody Operator](/docs/reference/appsody-operator) is used to monitor resources of kind `AppsodyApplication` and can be installed using the `appsody operator install` command. However, there might be cases where this command fails due to permissions problems in the cluster. 
